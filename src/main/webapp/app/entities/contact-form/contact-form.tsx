...
import { searchEntities, getEntities } from './contact-form.reducer';
...
export const ContactForm = () => {
  ...
  const contactFormList = useAppSelector(state => state.contactForm.entities);
    ...
    <div className="pt-20 px-4">
      <h2 id="contact-form-heading" data-cy="ContactFormHeading">
        ...
        <div className="d-flex justify-content-end p-3">
          ...
          <Link to="/contact-form/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="mortgageHubApp.contactForm.home.createLabel">Create new Contact Form</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('mortgageHubApp.contactForm.home.search')}
                />
        ...
        {contactFormList && contactFormList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="mortgageHubApp.contactForm.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('subscribe')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('subscribe')} />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {contactFormList.map((contactForm, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/contact-form/${contactForm.id}`} color="link" size="sm">
                      {contactForm.id}
                    </Button>
                  </td>
                  ...
                  <td>{contactForm.subscribe ? 'true' : 'false'}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/contact-form/${contactForm.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.view">View</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`/contact-form/${contactForm.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                      ...
                      <Button
                        onClick={() => (window.location.href = `/contact-form/${contactForm.id}/delete`)}
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="mortgageHubApp.contactForm.home.notFound">No Contact Forms found</Translate>
            </div>
          )
        )}
      </div>
